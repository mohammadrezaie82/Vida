<template>
  <v-app :theme="activeTheme">
    ...
    <v-app-bar app class="relative backdrop-blur-2xl shadow-2xl z-50">
      <!-- Gradient background -->
      <div
        class="absolute inset-0 -z-10 bg-gradient-to-br from-white via-cyan-50 to-blue-50 dark:from-cyan-900 dark:via-slate-900 dark:to-slate-950"
      ></div>

      <!-- Logo -->
      <div
        class="flex items-center gap-3 ms-6 cursor-pointer"
        @click="router.push('/dashboard/dashboard-home')"
      >
        <img :src="myLogo" alt="Logo" class="w-10 h-10 rounded-xl shadow-sm" />
        <span class="text-2xl font-bold text-cyan-500 dark:text-cyan-300"
          >Vida</span
        >
      </div>
      ...
      <div class="items-center gap-3 sm:gap-6 flex">
        ...
        <div
          v-if="userStore.user"
          class="hidden sm:flex items-center gap-2 text-sm px-3 py-1 rounded-xl text-slate-900 dark:text-white shadow"
        >
          <span
            >{{ userStore.user.province }} {{ userStore.user.city ? "," : "" }}
            {{ userStore.user.city }}</span
          >
          <v-icon size="18" class="text-cyan-500 dark:text-cyan-400"
        ...
        <v-menu offset-y>
          <template #activator="{ props }">
            ...
            >
              ...
              <img
                ...
                class="rounded-sm mr-2"
              />
              <span class="text-sm font-medium text-black dark:!text-gray-50">{{
                currentLanguageLabel
              }}</span>
              <v-icon size="18" class="ml-1 text-black dark:!text-gray-50"
          ...
          <div
            class="mt-2 w-36 rounded-3xl backdrop-blur-2xl bg-white/15 dark:bg-gray-900/30 border border-white/10 dark:border-gray-800 shadow-2xl p-2 flex flex-col gap-2"
          >
            <button
        ...
        <v-btn
          icon
          @click="toggleTheme"
          class="rounded-full p-2 shadow-lg hover:shadow-xl transition"
        >
          <v-icon v-if="isDark" class="text-yellow-400"
            >mdi-white-balance-sunny</v-icon
          >
          <v-icon v-else class="text-gray-700 dark:text-gray-200"
        ...
        <div v-if="userStore.user" class="flex items-center gap-2 me-6">
          <span
            class="font-medium text-slate-900 dark:text-white hidden lg:block"
    ...
    >
      <div
        class="absolute inset-0 -z-10 bg-gradient-to-b from-white via-cyan-50 to-blue-50 dark:from-cyan-900 dark:via-slate-900 dark:to-slate-950"
      ></div>
      ...
      <v-list density="comfortable" nav class="relative z-10 p-4">
        <v-list-item
          ...
          :key="i"
          :to="item.route"
          class="rounded-2xl mb-3 hover:bg-cyan-50 dark:hover:bg-gray-800 transition shadow justify-center lg:justify-start"
        >
          <div
            class="flex items-center gap-3 w-full text-slate-900 dark:text-white"
          >
            <div
              class="w-8 h-8 flex items-center justify-center rounded-full bg-cyan-100 dark:bg-gray-700 shadow"
            >
              <template
                v-if="
                  typeof item.icon === 'string' && item.icon.startsWith('mdi-')
                "
              >
                <v-icon class="text-cyan-600 dark:text-cyan-400">{{
                  item.icon
                }}</v-icon>
              </template>
              <template v-else>
...
const currentLanguageLabel = computed(() =>
  currentLanguage.value === "fa" ? "fa" : "en",
);
