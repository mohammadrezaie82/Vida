        ...
        <v-app-bar app class="relative backdrop-blur-2xl shadow-2xl z-50">
            ...
            <div class="items-center gap-3 sm:gap-6 flex">
                ...
                <div class="flex items-center gap-2 me-6">
                    <span class="font-medium text-slate-900 dark:text-white hidden lg:block">{{ user.name }}</span>
                    <div
...
watch(user, (newVal) => localStorage.setItem("user", JSON.stringify(newVal)), { deep: true });

window.addEventListener("storage", (event) => {
    if (event.key === "user" && event.newValue) Object.assign(user, JSON.parse(event.newValue));
});
